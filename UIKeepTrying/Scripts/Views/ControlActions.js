function ControlActions() {
    this.URL_API = "http://localhost:59462/api/";

    this.GetUrlApiService = function (service) {
        return this.URL_API + service;
    }

    this.GetTableColumnsDataName = function (tableId) { var val = $('#' + tableId).attr("ColumnsDataName"); return val; };

    this.FillTable = function (service, tableId, refresh)
    {
        if (!refresh) {
            columns = this.GetTableColumnsDataName(tableId).split(',');
